...
func autoComplete() -> void:
	...
	condition.set_comment_delimiters(["--"])
	
	var This = [":getVar()", ":setVar()", ".Instance", ".Name"]
	var Group = [":getVar()", ":setVar()"]
	var ThisDesc = [':getVar([variableName: String])', ':setVar([variableName: String], [data])', ".Instance", ".Name"]
	var GroupDesc = [':getVar([Instance: String], [variableName: String])', ':setVar([Targets: Array], [variableName: String], [data])']
...
func refreshList(selected: int = 0) -> void:
	...
	if MusGroups.client_logicList.size() > 0:
		loadLogic(selected)
...
func createLogic() -> void:
	...
	MusGroups.client_logicList[l.Instance] = {"Name": l.Name, "Instance": l.Instance}
